...
df = pd.read_csv('/big-mult/train/train_0.csv') #The dataset is downloaded with a big_mult folder name，unpack
# print(df.head())
df1 = pd.read_csv('../opcode_meaning.csv')
df['ops_new'] = 0
len_czm = len(df['bytecode'])

czm = df1['opcode']  #opcode
czm = list(czm)
hy = df1['meaning']    #The opcode corresponding to the bytecode 
...
df.to_csv('./yourcsv',index=False)  #save your dir
print(ans)
print(df.head())
