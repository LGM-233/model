...
wo2vec = KeyedVectors.load_word2vec_format("./smart300.bin")  #Word embedding
loss_func = nn.BCEWithLogitsLoss()   #Loss function
loss_ASL = AsymmetricLoss()   #The loss function is for an unbalanced dataset
...
class word2vec_add_biLSTM(nn.Module):
    def __init__(self, num_class, d_model, num_layer, input_size, hidden_size):
        super(word2vec_add_biLSTM, self).__init__()
        ...
        self.bilstm_fc = nn.Linear(4*300,6)
    ...
    def forward(self,x):
        ...
        end_out = L_out.reshape(1,4*300)

        logits = self.bilstm_fc(self.dropout(end_out))  
        logits = torch.squeeze(logits)  # [6]
        return logits

def get_split_input(x):  #split  data
    x_train = []
    x = x.split(',')
    len_x = len(x)
...
if __name__ == '__main__':
    ...
    model = word2vec_add_biLSTM(num_class=6, d_model=300, num_layer=1, input_size=300, hidden_size=300)  
